  ...
  const handleBulkAdd = (classId: string) => {
    ...
    const newStudents: Student[] = lines
      .map(line => {
        ...
        // Take the last word as surname, rest as first name
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="glass p-6 rounded-2xl border border-white/20 dark:border-slate-800 shadow-sm">
             ...
             <div className="flex gap-2">
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm transition-all shadow-md active:scale-95"
                >
                  +
                </button>
             </div>
              ...
              classes.map(c => (
                ...
                >
                   ...
                   <button 
                     ...
                     className="flex-shrink-0 p-2 text-slate-300 hover:text-rose-500 transition-colors bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-100 dark:border-slate-700 ml-4"
                   >
        ...
        <div className="lg:col-span-2">
          {activeClass ? (
             <div className="glass p-8 rounded-3xl border border-white/20 dark:border-slate-800 shadow-xl space-y-8 min-h-[500px]">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
                   ...
                   <button 
                     ...
                     className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl text-sm font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-700"
                   >
                      ...
                      {isBulkMode ? (
                        <div className="space-y-4">
                           ...
                           <textarea
                             ...
                             placeholder={t('classes.bulkPlaceholder')}
                             className="w-full h-48 px-4 py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-mono leading-relaxed resize-none"
                           />
                           <button 
                             onClick={() => handleBulkAdd(activeClass.id)}
                             disabled={!bulkText.trim()}
                             className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
                           >
                              {t('common.save')}
                           </button>
                        </div>
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="space-y-3">
                              <input 
                                type="text" 
                                placeholder={t('exam.firstName')}
                                value={newStudent.firstName}
                                onChange={(e) => setNewStudent(prev => ({...prev, firstName: e.target.value}))}
                                className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-medium"
                                onKeyPress={(e) => e.key === 'Enter' && handleAddStudent(activeClass.id)}
                              />
                              <input 
                                type="text" 
                                placeholder={t('exam.lastName')}
                        ...
                        activeClass.students.map(s => (
                          <div key={s.id} className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm hover:border-indigo-200 dark:hover:border-indigo-900 transition-all">
                             <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-full bg-indigo-50 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 text-xs font-bold uppercase">
                                  {s.firstName[0]}{s.lastName[0]}
                                </div>
                                <span className="font-semibold text-slate-700 dark:text-slate-200">{s.firstName} {s.lastName}</span>
                             </div>
                             <button 
                               onClick={() => handleDeleteStudent(activeClass.id, s.id)}
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400 p-20 glass rounded-3xl border border-dashed border-slate-200 dark:border-slate-800">
